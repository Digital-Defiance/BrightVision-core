...
- The agent prompt must state the edit contract (ResourceManager -> ReadFile -> EditFile,
  re-read after edits, empty-file markers) up front instead of relying on the harness to
...
def test_agent_prompt_states_edit_contract():
    ...
    for token in ("ReadFile", "EditFile", "ResourceManager"):
        assert token in text, f"agent main_system must mention {token}"
    lowered = text.lower()
    # ReadFile-before-EditFile ordering and the empty-file markers.
    assert "readfile" in lowered and "before every" in lowered
    assert "@000" in text and "000@" in text
    assert "~~" in text  # unique-line virtual prefix from current hashline format
